...
import { addMedicationApi, setMark } from "../api/api.js";
...
const PatientDashboard = () => {
  ...
  const [addMedication, setMedication] = useState({
    medicationName: "",
    dosage: "",
    frequency: "",
  });
  ...
  const handleMarkTaken = async (date: string, imageFile?: File) => {
    try {
      ...
      const res = await setMark(formData);
      setShowLoading(true);
      ...
      toast.error("🔥 Error marking medication:", err);
    }
  ...
  const fetchAddMedicationData = async (
    e: React.FormEvent<HTMLFormElement>
  ) => {
    e.preventDefault();
    const res = await addMedicationApi({ addMedication, userId });
    setShowLoading(true);
    if (res.status === 200) {
      setMedication({
        medicationName: "",
      ...
      setShouldFetch((perv) => perv + 1);
      toast.success("Medication Added Sccessfully");
    } else {
      toast.error(res);
    }
    setShowLoading(false);
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div>
          <Card>
            <CardContent>
              <form onSubmit={fetchAddMedicationData}>
                <div className="space-y-3 flex flex-col md:flex-row gap-2">
                  <input
                    type="text"
                    required
                    value={addMedication.medicationName}
                ...
                <button
                  type="submit"
                  className="mt-6 bg-green-600 hover:bg-green-700 text-white py-1 px-4 text-lg"
                >
                  Add Medication
                </button>
              </form>
