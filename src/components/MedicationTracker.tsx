...
import { getMedicationApi } from "../api/api.js";
...
const MedicationTracker = ({
  ...
  const getMedicationdata = async () => {
    const res = await getMedicationApi(patientId);
    setShowLoading(true);

    if (res.status === 200) {
      setMedicationList(res.data.medications);
      toast.success("Medication get Sccessfully");
    } else {
      toast.error(res);
    }
  ...
  const renderValue = () => {
    ...
    return medicationList.map((each, i) => (
      <p key={i}>
        Name: {each.medicationName} Dosage: {each.dosage} Frequency:{" "}
        {each.frequency}
      </p>
    ));
  };
